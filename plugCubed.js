...
define('plugCubed/Model',['jquery','underscore','app/base/Class','app/base/Context','app/facades/ChatFacade','app/store/LocalStorage','app/utils/Utilities','app/models/RoomModel','app/events/MediaCurateEvent','app/net/Socket','app/net/SocketIO','app/models/TheUserModel','lang/Lang','app/views/room/AudienceView','app/events/RoomJoinEvent','app/events/RoomStateEvent','plugCubed/StyleManager','app/views/room/user/RoomUserListView','app/views/room/user/RoomStaffListRow','plugCubed/RoomUserListRow','plugCubed/Lang','plugCubed/Utils','app/store/Database','app/models/PlaybackModel','plugCubed/dialogs/CustomChatColors','plugCubed/dialogs/Commands'],function($,_,Class,Context,Chat,LocalStorage,Utils,Room,MCE,Socket,SIO,TUM,Lang,Audience,RJE,RSE,Styles,RoomUserListView,RoomUserListRow,_RoomUserListRow,p3Lang,p3Utils,DB,PlaybackModel,dialogColors,dialogCommands) {
    ...
    function getUserInfo(data) {
        ...
        if (user === null) API.chatLog(p3Lang.i18n('error.userNotFound'));
            ...
            switch (user.status) {
                default:                  status = p3Lang.i18n('status.available'); break;
                case API.STATUS.AFK:      status = p3Lang.i18n('status.afk');       break;
                case API.STATUS.WORKING:  status = p3Lang.i18n('status.working');   break;
                case API.STATUS.SLEEPING: status = p3Lang.i18n('status.sleeping');  break;
            }

            switch (user.vote) {
                case -1:  voted = p3Lang.i18n('vote.meh');       break;
                default:  voted = p3Lang.i18n('vote.undecided'); break;
                case 1:   voted = p3Lang.i18n('vote.woot');      break;
            }
            ...
            p3Utils.appendChatMessage('<table style="width:100%;color:#CC00CC"><tr><td colspan="2"><strong>' + p3Lang.i18n('info.name') + '</strong>: <span style="color:#FFFFFF">' + Utils.cleanTypedString(user.username) + '</span></td></tr>' +
            ...
            '<tr><td colspan="2"><strong>' + p3Lang.i18n('info.ratio') + '</strong>: <span style="color:#FFFFFF">' + (function(a,b) { if (b === 0) return a === 0 ? '0:0' : '1:0'; for (var i = 1;i <= b;i++) { var e = i*(a/b); if (e%1 === 0) return e + ':' + i; } })(getUserData(user.id,'wootcount',0),getUserData(user.id,'mehcount',0)) + '</span></td></tr>' +
    ...
    function getAllUsers() {
        ...
        p3Utils.appendChatMessage($('<div/>').append(table).html());
    }
    ...
    function __init() {
        ...
        this.defaultAwayMsg = p3Lang.i18n('AFK.default');
        ...
        p3Utils.appendChatMessage(p3Lang.i18n('running',version) + '</span><br /><span class="chat-text" style="color:#66FFFF">' + p3Lang.i18n('commandsHelp'),'FFFF00');
        ...
        $('#chat-header').append(
            $('<div>').addClass('chat-header-button p3-s-stream').data('key','stream').click(this.proxy.onMenuButtonClick).mouseover(function() { Context.trigger('tooltip:show',p3Lang.i18n('menu.stream'),$(this),true); }).mouseout(function() { Context.trigger('tooltip:hide'); })
        ).append(
            $('<div>').addClass('chat-header-button p3-s-clear').data('key','clear').click(this.proxy.onMenuButtonClick).mouseover(function() { Context.trigger('tooltip:show',p3Lang.i18n('menu.clear'),$(this),true); }).mouseout(function() { Context.trigger('tooltip:hide'); })
        )
    ...
    return Class.extend({
        ...
        Socket: function() {
            ...
            socket.onmessage = function(msg) {
                ...
                if (type === 'update') {
                    ...
                    this.close();
                    API.chatLog(p3Lang.i18n('newVersion'), null, plugCubed.colors.infoMessage1);
                ...
                if (type === 'rave') {
                    if (p3Utils.isPlugCubedDeveloper(data.id) || p3Utils.isPlugCubedSponsor(data.id) || API.hasPermission(data.id,API.ROLE.HOST)) {
                        switch (data.value) {
                            case 0: return clearTimeout(Audience.strobeTimeoutID),Audience.lightsOut(false);
                            case 1: return Audience.strobeOnSpeed = 50,Audience.strobeOffSpeed = 160,Audience.strobeSwap();
                            case 2: return Audience.lightsOut(true);
                        }
        ...
        colorInfo: {
            ranks: {
                ...
                residentdj : { title: 'ranks.residentdj',   color: 'E90E82' },
        ...
        moderation: function(target, type) {
            if (API.hasPermission(undefined,API.ROLE.BOUNCER)) {
                ...
                switch (type) {
                    ...
                    default:         API.chatLog(p3Lang.i18n('error.unknownModeration')); return;
                }
        ...
        onMenuClick: function() {
            if (menuDiv !== undefined) {
                if (menuDiv.css('left') === '0px')
                    return menuDiv.animate({left: -270}), dialogColors.hide();
                return menuDiv.animate({left: 0});
            }
            menuDiv = $('<div id="p3-settings" style="left: -270px;">').append(
                            $('<div class="header">').append(
                                $('<div class="back">').append(
                                    $('<i class="icon icon-back"></i>')
                                ).click(function() { if (menuDiv !== undefined) menuDiv.animate({left: -270}); dialogColors.hide(); })
                            ).append(
                            ...
                            $('<div class="container">').append(
                                ...
                                .mouseover(function() { Context.trigger('tooltip:show','AFK Message',$(this),true); })
            ...
            switch (a) {
                ...
                case 'autorespond':
                    ...
                    if (this.settings.autorespond) {
                        ...
                        $('#chat-input-field').attr('disabled','disabled').attr('placeholder','Disable AFK Autorespond to chat');
                ...
                case 'afktimers':
                    ...
                    break;
                default: return API.chatLog(p3Lang.i18n('error.unknownMenuKey',a));
            }
            this.saveSettings();
        },
        /**
        ...
        onDjAdvance: function(data) {
            if ((this.settings.notify & 8) === 8)
                p3Utils.appendChatMessage(p3Lang.i18n('notify.message.stats',data.lastPlay.score.positive,data.lastPlay.score.negative,data.lastPlay.score.curates),this.settings.colors.stats);
            if ((this.settings.notify & 16) === 16)
                p3Utils.appendChatMessage(p3Lang.i18n('notify.message.updates',data.media.title,data.media.author,Utils.cleanTypedString(data.dj.username)),this.settings.colors.updates);
        ...
        onGrab: function(data) {
            ...
            if ((this.settings.notify & 4) === 4)
                p3Utils.appendChatMessage(p3Lang.i18n('notify.message.curate',Utils.cleanTypedString(data.user.username),media.author,media.title),this.settings.colors.curate);
        },
        /**
         * @this {plugCubedModel}
         */
        ...
        onUserJoin: function(data) {
            if ((this.settings.notify & 1) === 1)
                p3Utils.appendChatMessage(p3Lang.i18n('notify.message.join',Utils.cleanTypedString(data.username)),this.settings.colors.join);
        ...
        chatDisable: function(data) {
            var a = data.type == 'mention' && (API.hasPermission(data.fromID,API.ROLE.BOUNCER)),b = data.message.indexOf('@') < 0 && (API.hasPermission(data.fromID,API.ROLE.HOST) || p3Utils.isPlugCubedDeveloper(data.fromID));
            if (a || b) {
        ...
        onChat: function(data) {
            ...
            if (data.type == 'mention') {
                if (this.settings.autorespond && !this.settings.recent) {
                    ...
                    $('#chat-input-field').attr('placeholder','Next AFK Autorespond will be sent from ' + this.getTimestamp(Date.now() + 18E4));
                    setTimeout(function() { $('#chat-input-field').attr('placeholder','AFK Autorespond on next mention'); plugCubed.settings.recent = false; plugCubed.saveSettings(); },18E4);
        ...
        onWaitlistShow: function() {
            setTimeout(function() {
                console.log('Waitlist Show');
                $('#waitlist .list .user').each(function() {
                    $(this).find('.meta').append($('<span>').addClass('afk'));
        ...
        onHistoryCheck: function(id) {
            ...
            for (var i in p3history) {
                ...
                if (a.id == id && (~~i + 1) < p3history.length) {
                    found = ~~i + 2;
                    if (!a.wasSkipped)
                        return playMentionSound(),setTimeout(function() { playMentionSound() },50),API.chatLog(p3Lang.i18n('historyCheck.inHistory',found,p3history.length),true);
                }
            }
            if (found > 0)
                return API.chatLog(p3Lang.i18n('historyCheck.inHistorySkipped',found,p3history.length),true);
        },
            ...
            if (API.hasPermission(undefined,API.ROLE.HOST) || p3Utils.isPlugCubedDeveloper()) {
                if (value.indexOf('/strobe') === 0) {
                    ...
                    if (value.indexOf('off') > -1)
                        ...
                        socket.send(JSON.stringify({type:'rave',value:~~args[1]}));
                    else
        ...
        GUIInput: function(id,text,defaultColor) {
            return $('<div class="item">').addClass('p3-s-cc-' + id).append(
                ...
                $('<span>').addClass('default').css('display',plugCubed.settings.colors[id] === defaultColor ? 'none' : 'block')
                    .mouseover(function() { d.trigger('tooltip:show','Reset to default',$(this),true); })
                    .mouseout(function() { d.trigger('tooltip:hide'); })
                    .click(function() {
        ...
        render: function() {
            if (div !== undefined) {
                if (div.css('left') === '-270px')
                    return div.animate({left: $('#p3-settings').width() + 1});
                return div.animate({left: -270});
            }
            ...
            div = $('<div id="p3-settings-custom-colors" style="left: -270px;">').append(
                    $('<div class="header">').append(
                        $('<div class="back">').append(
                            $('<i class="icon icon-back"></i>')
                        ).click(function() { if (div !== undefined) div.animate({left: -270}); })
                    ).append(
                        $('<div class="title">').append(
                            $('<span>').text(p3Lang.i18n('menu.customchatcolors'))
                        )
                    )
                ).append(container).animate({left: $('#p3-settings').width() + 1});
            $('body').append(div);
        },
        hide: function() {
            if (div !== undefined) div.animate({left: -270});
        }
    });
    return new a();
});
define('plugCubed/dialogs/Userinfo',['jquery','app/views/dialogs/AbstractDialogView','lang/Lang','app/base/Class','app/base/Context','app/events/ShowDialogEvent','plugCubed/Lang'],function($,b,c,d,e,f,p3Lang) {
    var a = d.extend({
        init: function(id) {
...
define('plugCubed/dialogs/Commands',['jquery','app/base/Class','lang/Lang','plugCubed/Lang','plugCubed/Utils'],function($,b,c,p3Lang,p3Utils) {
    var userCommands = [
    ...
    a = b.extend({
        print: function() {
            var content = '<strong style="position:absolute;left: 5px;">=== User Commands ===</strong><br />';
            for (var i in userCommands)
                content += '<span style="position:absolute;left: 20px;">' + userCommands[i][0] + '</span><br /><em style="position:absolute;left: 45px;">' + userCommands[i][1] + '</em><br />';
            if (API.hasPermission(undefined,API.ROLE.BOUNCER)) {
                content += '<br /><strong style="position:absolute;left: 5px;">=== Moderation Commands ===</strong><br />';
                for (var i in modCommands) {
                    if (API.hasPermission(undefined,modCommands[i][2]))
                        content += '<span style="position:absolute;left: 20px;">' + modCommands[i][0] + '</span><br /><em style="position:absolute;left: 45px;">' + modCommands[i][1] + '</em><br />';
                }
            }
            p3Utils.appendChatMessage(content);
        }
    });
    return new a();
});
define('plugCubed/StyleManager',['jquery','app/base/Class'],function($,Class) {
    var obj,
        styles = {},
        update = function() {
            var a = '';
...
define('plugCubed/Utils',function() {
    return {
        ...
        appendChatMessage: function(message,color) {
            if (!message) return;
            if (typeof message !== 'string') message = message.html();
            message = message.split('<script').join('');
            var a = $('#chat-messages'),b = a.scrollTop() > a[0].scrollHeight - a.height() - 20;
            a.append('<div class="update"><span class="text" style="color:#' + (color ? color : 'd1d1d1') + '">' + message + '</span></div>');
            b && a.scrollTop(a[0].scrollHeight);
        }
    };
});
define('plugCubed/RoomUserListRow',['jquery','app/views/room/user/RoomUserListRow','plugCubed/Utils'],function($,RoomUserListRow,p3Utils) {
...
define('plugCubed/MenuView',['jquery','underscore','backbone','app/base/Context'], function($,_,Backbone,Context) {
    var a = Backbone.View.extend({
        id: 'plug-cubed',
        initialize: function() {
            this.itemBind = _.bind(this.onItemClick, this);
            this.documentBind = _.bind(this.onDocumentClick, this);
            this.showing = false;
        },
        render: function() {
            this.$el.html();
            this.$el.on('mousedown', _.bind(this.onButtonClick, this));
            this.$menu = this.$('#plug-cubed').remove();
            return this;
        },
        ...
        show: function() {
            this.showing = true;
            Context.trigger('settings:hide')
                   .trigger('playback:block');
            $('body').append(this.$menu);
            $('#plug-cubed .clickable').on('mousedown', this.itemBind);
            var self = this;
            _.defer(function(){$(document).on('mousedown', self.documentBind);})
        },
        hide: function() {
            this.showing = false;
            $(document).off('mousedown', this.documentBind);
            $('#plug-cubed .item').off('mousedown', this.itemBind);
            Context.trigger('playback:unblock');
            this.$menu.remove();
        },
        onItemClick: function(event) {
            var target = $(event.target);
            if (target.is('span')) target = target.parent();
            //if (target.hasClass('room-list')) Context.trigger('lobby:show');
            //else if (target.hasClass('settings')) Context.trigger('settings:show');
            //else if (target.hasClass('help')) console.log('help!');
        },
        onDocumentClick: function(event) {
            var $target = $(event.target);
            if ($target.is('span')) $target = $target.parent();
            if (!$target.is('a') && !$target.hasClass('item')) this.hide();
            else {
                var self = this;
                _.delay(function(){self.hide();}, 250);
            }
        }
    });
    return PlugMenuView;
});
define('plugCubed/Lang',['jquery','app/base/Class'],function($,Class) {
    var language = {},
        isLoaded = false,
        Lang = Class.extend({
            init: function() {
            ...
            load: function(lang,callback) {
                if (!isLoaded) {
                    var _this = this;
                    return setTimeout(function() { _this.load(lang,callback); },500);
                }
                if (!lang || lang === 'en') {
...
define('plugCubed/Loader',['jquery','app/base/Class','plugCubed/Model','app/store/LocalStorage','app/views/dialogs/AbstractDialogView','plugCubed/Lang','plugCubed/Utils'],function($,Class,Model,LocalStorage,ADV,p3Lang,p3Utils) {
        ...
        render: function() {
            ...
            $.getJSON('http://alpha.plugcubed.net/lang.json',function(data) {
                ...
                this.$el.append(this.getHeader('plug&#179; language'))
                    ...
                    return this._super();
            }).done(function() { if (self.languages.length === 0) p3Utils.appendChatMessage('<span style="color:#FF0000">Error loading plugCubed</span>'); });
            /*
