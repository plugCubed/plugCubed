...
String.prototype.toRGB                = function()    { return this.isRGB() ? (this.substr(0,1) === '#' ? this : '#' + this) : undefined; };
...
(function() {
    ...
    define('plugCubed/Model',['jquery','underscore','app/base/Class','app/base/Context','app/facades/ChatFacade','app/store/LocalStorage','app/utils/Utilities','app/models/RoomModel','app/events/MediaCurateEvent','app/net/Socket','app/net/SocketIO','app/models/TheUserModel','lang/Lang','app/views/room/AudienceView','app/events/RoomJoinEvent','app/events/RoomStateEvent','plugCubed/StyleManager','app/views/room/user/RoomUserListView','app/views/room/user/RoomStaffListRow','plugCubed/RoomUserListRow','plugCubed/Lang','plugCubed/Utils','app/store/Database','app/models/PlaybackModel','plugCubed/dialogs/Commands','plugCubed/Slider','plugCubed/VolumeView','plugCubed/dialogs/CustomChatColors'],function($,_,Class,Context,Chat,LocalStorage,Utils,Room,MCE,Socket,SIO,TUM,Lang,Audience,RJE,RSE,Styles,RoomUserListView,RoomUserListRow,_RoomUserListRow,p3Lang,p3Utils,Database,PlaybackModel,dialogCommands,Slider,VolumeView,dialogColors) {
        ...
        function getUserInfo(data) {
            ...
            if (user === null) API.chatLog(p3Lang.i18n('error.userNotFound'));
                ...
                var rank,
                    ...
                    disconnectInfo = getUserData(user.id,'disconnects',{count:0});
                ...
                switch (user.status) {
                    ...
                    case API.STATUS.GAMING:   status = p3Lang.i18n('status.gaming');    break;
                }
        ...
        function afkTimerTick() {
            if (plugCubed.settings.afkTimers && (p3Utils.isPlugCubedDeveloper() || API.hasPermission(undefined,API.ROLE.BOUNCER)) && $('#waitlist-button').hasClass('selected')) {
                var a = API.getWaitList(),b = $('#waitlist .user');
        ...
        function antiDangerousScripts() {
            ...
            for (var b in a) {
                ...
                if ((function(words) { for (var i in words) { if (script.indexOf(words[i]) > -1) return true; } return false; })(['API.djLeave','API.djJoin','API.moderateLockWaitList','API.moderateForceSkip','.getScript(']))
                    return c();
            }
        ...
        function __init() {
            ...
            $('#chat-header').append(
                $('<div>').addClass('chat-header-button p3-s-stream').data('key','stream').click(this.proxy.onMenuButtonClick).mouseover(function() { Context.trigger('tooltip:show',p3Lang.i18n('tooltip.stream'),$(this),true); }).mouseout(function() { Context.trigger('tooltip:hide'); })
            ).append(
        ...
        return Class.extend({
            ...
            onRoomJoin: function() {
                if (typeof plugCubed !== 'undefined') {
                    setTimeout(function() {
                        if (API.enabled) $.getScript('https://d1rfegul30378.cloudfront.net/alpha/plugCubed.' + (version.minified ? 'min.' : '') + 'js');
            ...
            Socket: function() {
                ...
                socket.onmessage = function(msg) {
                    ...
                    if (type === 'update') {
                        ...
                        return setTimeout(function() { $.getScript('https://d1rfegul30378.cloudfront.net/alpha/plugCubed.' + (version.minified ? 'min.' : '') + 'js'); },5000);
                    }
                    if (type === 'chat') {
            ...
            colorInfo: {
                ranks: {
                    ...
                    residentdj : { title: 'ranks.residentdj',   color: 'AC76FF' },
                    bouncer    : { title: 'ranks.bouncer',      color: 'AC76FF' },
                    manager    : { title: 'ranks.manager',      color: 'AC76FF' },
                    cohost     : { title: 'ranks.cohost',       color: 'AC76FF' },
                    host       : { title: 'ranks.host',         color: 'AC76FF' },
                    ambassador : { title: 'ranks.ambassador',   color: '89BE6C' },
                ...
                notifications: {
                    ...
                    songLength : { title: 'notify.songLength',  color: '66FFFF' }
                }
            },
            settings: {
                recent           : false,
            ...
            updateCustomColors: function() {
                ...
                if ((b && roomChatColors.regular) || this.settings.colors.regular !== this.colorInfo.ranks.regular.color)
                    a.push('.user > .name:first-child,', '.message.from > .from,','.emote.from > .from,','.mention.from > .from { color:#' + (this.settings.colors.regular !== this.colorInfo.ranks.regular.color ? this.settings.colors.regular : roomChatColors.regular) + '!important; }');
                if ((b && roomChatColors.residentdj) || this.settings.colors.residentdj !== this.colorInfo.ranks.residentdj.color)
                    a.push('.user > .icon-chat-dj + .name,', '.message.from-residentdj > .from,','.emote.from-residentdj > .from,','.mention.from-residentdj > .from { color:#' + (this.settings.colors.residentdj !== this.colorInfo.ranks.residentdj.color ? this.settings.colors.residentdj : roomChatColors.residentdj) + '!important; }');
                if ((b && roomChatColors.bouncer) || this.settings.colors.bouncer !== this.colorInfo.ranks.bouncer.color)
                    a.push('.user > .icon-chat-bouncer + .name,', '.message.from-bouncer > .from,','.emote.from-bouncer > .from,','.mention.from-bouncer > .from { color:#' + (this.settings.colors.bouncer !== this.colorInfo.ranks.bouncer.color ? this.settings.colors.bouncer : roomChatColors.bouncer) + '!important; }');
                if ((b && roomChatColors.manager) || this.settings.colors.manager !== this.colorInfo.ranks.manager.color)
                    a.push('.user > .icon-chat-manager + .name,', '.message.from-manager > .from,','.emote.from-manager > .from,','.mention.from-manager > .from { color:#' + (this.settings.colors.manager !== this.colorInfo.ranks.manager.color ? this.settings.colors.manager : roomChatColors.manager) + '!important; }');
                if ((b && roomChatColors.cohost) || this.settings.colors.cohost !== this.colorInfo.ranks.cohost.color)
                    a.push('.user > .icon-chat-cohost + .name,', '.message.from-cohost > .from,','.emote.from-cohost > .from,','.mention.from-cohost > .from { color:#' + (this.settings.colors.cohost !== this.colorInfo.ranks.cohost.color ? this.settings.colors.cohost : roomChatColors.cohost) + '!important; }');
                if ((b && roomChatColors.host) || this.settings.colors.host !== this.colorInfo.ranks.host.color)
                    a.push('.user > .icon-chat-host + .name,', '.message.from-host > .from,','.emote.from-host > .from,','.mention.from-host > .from { color:#' + (this.settings.colors.host !== this.colorInfo.ranks.host.color ? this.settings.colors.host : roomChatColors.host) + '!important; }');
                if ((b && roomChatColors.ambassador) || this.settings.colors.ambassador !== this.colorInfo.ranks.ambassador.color)
                    a.push('.user > .icon-chat-ambassador + .name,', '.message.from-ambassador > .from,','.emote.from-ambassador > .from,','.mention.from-ambassador > .from { color:#' + (this.settings.colors.ambassador !== this.colorInfo.ranks.ambassador.color ? this.settings.colors.ambassador : roomChatColors.ambassador) + '!important; }');
                if ((b && roomChatColors.admin) || this.settings.colors.admin !== this.colorInfo.ranks.admin.color)
                    a.push('.user > .icon-chat-admin + .name,', '.message.from-admin > .from,','.emote.from-admin > .from,','.mention.from-admin > .from { color:#' + (this.settings.colors.admin !== this.colorInfo.ranks.admin.color ? this.settings.colors.admin : roomChatColors.admin) + '!important; }');
            ...
            onMenuClick: function() {
                if (menuDiv !== undefined) {
                    if (menuDiv.css('left') === '0px')
                        return menuDiv.animate({left: -271}), dialogColors.hide();
                ...
                menuDiv = $('<div id="p3-settings" style="left: -271px;">').append(
                                $('<div class="header">').append(
                                    ...
                                    ).click(function() { if (menuDiv !== undefined) menuDiv.animate({left: -271}); dialogColors.hide(); })
                                ).append(
            ...
            onChatCommand: function(value) {
                ...
                if (API.hasPermission(undefined,API.ROLE.COHOST) || p3Utils.isPlugCubedDeveloper() || p3Utils.isPlugCubedSponsor()) {
                    ...
                    if (value.indexOf('/rave') === 0) {
                        if (socket.readyState !== SockJS.OPEN) return API.chatLog(p3Lang.i18n('error.notConnected'),true);
                        socket.send(JSON.stringify({type:'rave',value:value.indexOf('off') > -1 ? 0 : 2}));
                    }
                }
                if (API.hasPermission(undefined,API.ROLE.MANAGER)) {
                    if (value === '/lock')
    ...
    define('plugCubed/dialogs/CustomChatColors',['jquery','app/base/Class','lang/Lang','app/base/Context','plugCubed/Lang'],function($,b,c,d,p3Lang) {
        function GUIInput(id,text,defaultColor) {
            return $('<div class="item">').addClass('p3-s-cc-' + id).append(
                ...
                $('<span>').addClass('default').css('display',plugCubed.settings.colors[id] === defaultColor ? 'none' : 'block')
                    .mouseover(function() { d.trigger('tooltip:show',p3Lang.i18n('tooltip.reset'),$(this),false); })
        ...
        var div,a = b.extend({
            render: function() {
                ...
                div = $('<div id="p3-settings-custom-colors" style="left: -271px;">').append(
                        $('<div class="header">').append(
                            $('<div class="back">').append(
                                $('<i class="icon icon-back"></i>')
                            ).click(function() { if (div !== undefined) div.animate({left: -271}); })
                        ).append(
                            $('<div class="title">').append(
            ...
            hide: function() {
                if (div !== undefined) div.animate({left: -271});
            }
        });
        return new a();
    });
            ...
            render: function() {
                ...
                $.getJSON('https://d1rfegul30378.cloudfront.net/alpha/lang.json',function(data) {
                    ...
                    this.$el.append(this.getHeader('plug&#179; language'))
                        ...
                        return this._super();
                }).done(function() { if (self.languages.length === 0) p3Utils.chatLog(undefined,'<span style="color:#FF0000">Error loading plugCubed</span>'); });
                /*
