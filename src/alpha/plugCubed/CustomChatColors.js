    ...
    var handler = Class.extend({
        update: function() {
            ...
            if ((userRoomSettings && RSS.chatColors.you) || Settings.colors.you !== Settings.colorInfo.ranks.you.color) {
                Styles.set('CCC-text-you', ['#user-lists .list .user .name', '.message > .from', '.emote > .from', '.mention > .from { color:' + p3Utils.toRGB(Settings.colors.you !== Settings.colorInfo.ranks.you.color ? Settings.colors.you : RSS.chatColors.you) + '!important; }'].join(",\n"));
            }
            if (userRoomSettings) {
